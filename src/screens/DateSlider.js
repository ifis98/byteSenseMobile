...
const DateSlider = props => {
  ...
  const dates = props?.healthData.map(data => moment(data.Date, 'YYYY-MM-DD')); // Ensure the selected date is always in the center
  const today = moment().format('YYYY-MM-DD');
  const filteredDates = props?.healthData
    .map(data => ({
      ...data,
      Date: moment(data.Date, 'YYYY-MM-DD'), // Parse the date properly
    }))
    .filter(data => data.Date.isSameOrBefore(today, 'day')) // Keep only today and past dates
    .sort((a, b) => b.Date.diff(a.Date)); // Sort in descending order

  console.log('filteredDatesfilteredDatesfilteredDates', filteredDates);
