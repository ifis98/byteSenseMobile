...
const AddDeviceScreen = () => {
  ...
  //     try {
  //       const bluetoothScanGranted = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.BLUETOOTH_SCAN);
  //       const bluetoothConnectGranted = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.BLUETOOTH_CONNECT);
  ...
  //         startScan(); // Permissions are already granted, start scanning
  //       } else {
...
const openAppSettings = () => {
  Alert.alert(
    ...
    'You need to enable Bluetooth and Location permissions from the app settings.',
    [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Open Settings', onPress: () => Linking.openSettings() }, // Open app settings for the user to manually enable
    ],
    { cancelable: false }
  );
};
  ...
  if (Platform.OS === 'android') {
    try {
      ...
      if (
        ...
        Alert.alert('Permissions required', 'Please grant Bluetooth and Location permissions for the app to work properly.');
      } else if (
        ...
        openAppSettings(); // Direct the user to open app settings if "Don't ask again" was selected
      }
    } catch (err) {
      console.warn(err);
    }
