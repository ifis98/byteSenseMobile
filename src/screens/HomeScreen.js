...
import healthData2 from '../hard_data/DashboardData.json'; // Import your health data JSON
import moment from 'moment';

const healthData = healthData2.sort(
  (a, b) => new Date(b.Date) - new Date(a.Date),
);
const {width} = Dimensions.get('window');

const updateLatestDate = data => {
  ...
  // Update the latest entry with today's date
  data[0].Date = today;
...
const SplashScreen = () => {
  // Assuming props?.healthData contains the JSON data
  const getLatestByteScores = healthData => {
    if (!healthData || healthData.length === 0) return [0, 0, 0, 0, 0, 0, 0];

    // Sort data in descending order by Date (latest first)
    const sortedData = healthData.sort((a, b) =>
      moment(b.Date, 'YYYY-MM-DD').diff(moment(a.Date, 'YYYY-MM-DD')),
    );

    // Extract the latest 7 byteScore values (or fill remaining with 0 if less than 7 entries)
    const latestByteScores = sortedData.slice(0, 7).map(item => item.byteScore);

    // Ensure the array has exactly 7 values, filling with 0 if necessary
    while (latestByteScores.length < 7) {
      latestByteScores.push(0);
    }

    return latestByteScores;
  };

  // Example Usage
  const dailyByteScoreData = getLatestByteScores(healthData);
  // console.log('dailyByteScoreData', dailyByteScoreData);
  const navigation = useNavigation();

  // const updatedHealthData = healthData;
  console.log('healthDatahealthDatahealthDatahealthData', healthData);
