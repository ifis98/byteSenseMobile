...
const CustomSVG = ({ angle, selected, index }) => {
  ...
  const y = radius * Math.sin(angle) + radius - (selected ? selectedHeight : baseHeight) / 2;
  ...
  const containerStyle = {
    ...
    backgroundColor: selected ? 'rgba(255, 255, 255, 1)' : index <= 5 || index >= 45 ? 'rgba(255, 255, 255, 0.3)' : 'rgba(255, 255, 255, 0.4)',
    borderRadius: 35,
...
const HalfCircleSVGs = (props) => {
  const items = Array.from({ length: 50 }).map((_, index) => {
    ...
    return <CustomSVG key={index} index={index + 1} angle={angle} selected={index === props.selectedIndex / 2} />;
  });
